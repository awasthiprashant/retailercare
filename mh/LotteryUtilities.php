...
class LotteryUtilities {
        ...
        elseif (strtolower($row["freq"]) == "monthly" || strtolower($row["freq"]) == "bumper") 
	{
	     ...
	     {
               /* if( date('n', strtotime($drawdate))  <=  date('n', strtotime($row["drawdate"])) )
                        return ($row["drawno"] + 12 + (date('n', strtotime($drawdate)) - date('n', strtotime($row["drawdate"]))));
                else
	                return ($row["drawno"] + (date('n', strtotime($drawdate)) - date('n', strtotime($row["drawdate"]))));*/

		 $diff = DateDif($row["drawdate"],$drawdate);
